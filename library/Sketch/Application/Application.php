...
use Sketch\Form\FormNotice;
use Sketch\Locale\Locale;
use Sketch\Logger\Logger;
use Sketch\Logger\MockLogger;
use Sketch\Logger\SimpleLogger;
use Sketch\HTTP\Request;
use Sketch\Resource\Database\Database;
use Sketch\Resource\Database\Exception;
use Sketch\Resource\Factory;
use Sketch\Resource\Context;
use Sketch\HTTP\Response\Response;
use Sketch\HTTP\JSON;
use Sketch\Core\Session\Session;
use Sketch\Core\Session\SessionACL;
use Sketch\Core\Session\SessionMock;
...
class Application {
    ...
    /**
     * @var \Sketch\Resource\Database\Database
     */
    private $connection;

    /**
     * @var \Sketch\Application\Controller\Controller
     */
    private $controller;

    /**
     * @var \Sketch\HTTP\Request
     */
    private $request;

    /**
     * @var \Sketch\Core\\Sketch\Core\Session\Session
     */
    private $session;
    ...
    function load($document_root, $controller = true, $test = false, $context = null) {
        if ($this->isLoaded) {
            ...
            set_error_handler(array('Sketch\Application\Application', 'exceptionErrorHandler'));
            $this->setStartTime(microtime(true));
            $this->setDocumentRoot($document_root);
            $this->setContext(
                Factory::getContext($context ? $context : dirname($document_root).DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.'context.xml')
            );
            // Initialize request
            ...
            try {
                $this->setConnection(
                    Factory::getConnection($this->getContext())
                );
            } catch (Exception $e) {}
            if ($controller) {
    ...
    /**
     * @param \Sketch\Logger\Logger $logger
     */
    ...
    /**
     * @return \Sketch\Resource\Database\Database
     */
     ...
     */
    function setConnection(Database $connection = null) {
    ...
    /**
     * @param \Sketch\Application\Controller\Controller $controller
     */
