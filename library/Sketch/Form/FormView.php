...
use Sketch\Core\ObjectList;
use Sketch\Core\ObjectView;
...
class FormView extends Object {
    ...
    function __call($name, $arguments) {
        ...
        $name = implode('', $split);
        if (class_exists('Sketch\Form\Component\\'.$name)) {
            $reflection = new \ReflectionClass('Sketch\Form\Component\\'.$name);
            if ($reflection->isSubclassOf('Sketch\Form\Component\\')) {
                /** @var \Sketch\Form\Component\Component $instance */
    ...
    function setFieldValue($ape, $value) {
        ...
        if ($value instanceof Descriptor && method_exists($instance, "addDescriptor")) {
            ...
            if ($instance instanceof ObjectView || $instance instanceof Descriptor) {
