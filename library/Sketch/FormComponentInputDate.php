...
class FormComponentInputDate extends FormComponent {
    ...
    function saveHTML() {
        ...
        $parameters = $this->extend(array(
            ...
            'from_date' => new DateTime('2010-01-01'),
            'to_date' => DateTime::Today()->addInterval('24 months'),
            'calendar' => false,
        ...
        if ($parameters['from_current_date']) {
            $parameters['from_date'] = DateTime::Today();
        }
        if ($parameters['to_current_date']) {
            $parameters['to_date'] = DateTime::Today();
            $parameters['from_date'] = $parameters['to_date']->addInterval('-'.$parameters['month_count'].' months');
        } else {
            if (!($parameters['from_date'] instanceof DateTime)) {
                $month_count = ceil($parameters['month_count'] / 2);
                $parameters['from_date'] = DateTime::Today()->addInterval('-'.$month_count.' months');
            }
            if (!($parameters['to_date'] instanceof DateTime)) {
