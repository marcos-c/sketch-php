...
use Sketch\ReflectionMethod;
use Sketch\ResourceXML;
use Sketch\HTTP\Response\Part;

/**
 * Template HTTP response filter
 *
    ...
    function applyForURI(ResourceXML $resource, $uri) {
        ...
        if ($q instanceof \DOMNodeList) foreach ($q as $template) {
            if ($template->hasAttribute('src')) {
                $template = Part::evaluate($template_path.DIRECTORY_SEPARATOR.$template->getAttribute('src'), false, $attributes, true);
