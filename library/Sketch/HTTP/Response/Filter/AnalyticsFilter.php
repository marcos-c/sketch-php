...
/**
 * Analytics HTTP response filter
 *
 * @package Sketch\HTTP\Response\Filter
 */
class AnalyticsFilter extends Filter {
    /**
    ...
    function apply(ResourceXML $resource) {
        ...
        foreach ($r as $transaction_handler) {
            ...
            if (class_exists($class)) {
                ...
                if ($reflection->implementsInterface('Sketch\Response\Filter\AnalyticsTransactionHandlerInterface')) {
                    $script .= $reflection->newInstance()->execute();
                } else {
                    throw new \Exception(sprintf($this->getTranslator()->_s("Handler %s does not implement AnalyticsTransactionHandlerInterface"), $class));
                }
            } else {
