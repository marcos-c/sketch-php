...
class ResourceFolder extends Resource {
    ...
    function __construct($name, $parent_id) {
        ...
        if ($parent_id = $this->getParentId()) {
            ...
            foreach ($connection->executeQuery("SELECT * FROM $table WHERE parent_id = $parent_id ORDER BY source_file_name") as $current) {
                ...
                $this->descriptors[$current['reference']] = new ResourceFolderDescriptor($current);
            }
        }
    }
     ...
     * @return ResourceFolderDescriptor
     */
    function getDescriptor($reference) {
        return (array_key_exists($reference, $this->descriptors) && $this->descriptors[$reference] instanceof ResourceFolderDescriptor) ? $this->descriptors[$reference] : false;
    }
    ...
    /**
     * @return ResourceFolderDescriptorList
     */
    function getDescriptorList() {
        return new ResourceFolderDescriptorList($this->descriptors);
    }
    ...
    /**
     * @param ResourceFolderDescriptor $descriptor
     */
    function addDescriptor($descriptor) {
        if (($descriptor instanceof ResourceFolderDescriptor) && is_readable($descriptor->getFileName())) {
     ...
     * @return boolean
     */
    function updateDescriptor(ResourceFolderDescriptor $descriptor) {
        $connection = $this->getConnection();
    ...
    private function clean() {
        ...
        foreach ($connection->executeQuery("SELECT * FROM $table") as $current) {
            ...
            $descriptor = new ResourceFolderDescriptor($current);
